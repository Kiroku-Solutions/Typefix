...
pub use language::{detector::*, stopwords::*};
...
static ENGINE_STATE: Lazy<Arc<RwLock<EngineState>>> = Lazy::new(|| {
...
pub fn init(config: &core::config::Config) -> Result<()> {
    ...
    tracing::info!("Engine initialized with {} languages", config.supported_languages.len());
    Ok(())
}
...
fn load_language_data(
    ...
    // Load dictionary
    let dict_path = data_path.join("dictionaries").join(format!("{}.json", lang));
    if dict_path.exists() {
    ...
    if stopwords_path.exists() {
        ...
        state.stopwords.insert(lang.to_string(), Arc::new(stopwords));
    ...
    if errors_path.exists() {
        ...
        state.error_maps.insert(lang.to_string(), Arc::new(error_map));
