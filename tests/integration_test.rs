...
use typefix::hooks::platform::{HookConfig, KeyEvent, SpecialKey, HookEvent, MockHook, HookMode, KeyboardHook};
use typefix::pipeline::{TypeFixPipeline, PipelineConfig, PipelineEvent, PipelineResult};
...
fn test_editor_simulator_language_detection() {
    ...
    let has_language_detection = results.iter()
        .any(|r| r.detected_language.is_some());
...
fn test_hook_editor_special_keys() {
    ...
    let events = editor.get_received_events();
    assert!(events.iter().any(|e| matches!(e.event, KeyEvent::Special(SpecialKey::Enter))));
...
fn test_hook_editor_tab_key() {
    ...
    let events = editor.get_received_events();
    assert!(events.iter().any(|e| matches!(e.event, KeyEvent::Special(SpecialKey::Tab))));
...
fn test_hook_editor_backspace() {
    ...
    let events = editor.get_received_events();
    assert!(events.iter().any(|e| matches!(e.event, KeyEvent::Special(SpecialKey::Backspace))));
...
fn test_editor_email_scenario() {
    ...
    let has_teh_correction = results.iter()
...
fn test_language_detection_sufficient_data() {
    ...
    let result = detector.process_word("makes");
