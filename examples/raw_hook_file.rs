...
unsafe extern "system" fn hook_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    if code >= 0 {
        ...
        if let Ok(mut file) = OpenOptions::new().create(true).append(true).open("hook_log.txt") {
            let _ = writeln!(file, "HOOK EVENT: vkCode={}, type={}", kb.vkCode, kind);
        }
    }
...
fn main() {
    if let Ok(mut file) = OpenOptions::new().create(true).append(true).open("hook_log.txt") {
        let _ = writeln!(file, "Starting raw keyboard hook test...");
    }
    ...
    unsafe {
        ...
        if hook.is_ok() {
            if let Ok(mut file) = OpenOptions::new().create(true).append(true).open("hook_log.txt") {
                let _ = writeln!(file, "Hook installed successfully! Try typing now...");
            }
        }
